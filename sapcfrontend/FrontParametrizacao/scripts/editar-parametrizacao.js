            ...
            .then((result) => {
                ...
                empresaProfileSection.innerHTML = /*html*/`
                    <div class="bg-white shadow-md rounded-lg p-6 max-w-sm mx-auto">
                        ...
                        <p class="bottom-0 right-0 text-sm"><strong>Data de Criação:</strong> ${formatarData(result.dataCriacao)}</p>
        ...
        .then((result) => {
            ...
            empresaProfileSection.innerHTML = /*html*/`
                <form onsubmit="atualizarParametrizacao(event)" id="param">
                    ...
                    <div class="mb-4">
                        <label for="logoGrande" class="block text-gray-700">Logo Grande:</label>
                        <input type="text" id="logoGrande" name="logoGrande" required
                            class="w-full px-3 py-2 rounded-lg border-gray-300 focus:border-blue-500 focus:outline-none"
                            value="${result.logoGrande}">
                    </div>
                    <div class="mb-4">
...
function atualizarParametrizacao(event) {
    ...
    fetch("http://localhost:8080/api/empresa/update", requestOptions)
        .then(() => {
            window.location.href = "perfil.html"
        })
}